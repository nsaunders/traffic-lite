...
newtype UpdateM a = UpdateM (ReaderT { path :: FilePath, repo :: String, token :: String } (ExceptT TrafficLite.Error Aff) a)
...
derive newtype instance MonadAsk { path :: FilePath, repo :: String, token :: String } UpdateM
derive newtype instance MonadEffect UpdateM
derive newtype instance MonadThrow TrafficLite.Error UpdateM

runUpdateM :: forall r a. UpdateM a -> { path :: FilePath, repo :: String, token :: String | r } -> ExceptT TrafficLite.Error Aff a
runUpdateM (UpdateM reader) { path, repo, token } = runReaderT reader { path, repo, token }
